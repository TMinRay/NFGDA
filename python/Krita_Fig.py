...
def nffig_proc(case_name,eval_on=False):
    ...
    for ic,data in enumerate(wgfspace.data):
        ...
        axs[0].plot(Cx[nfloc],Cy[nfloc],'.',color=(1,0,1),markersize=0.4)
        if eval_on:
            handpick = scipy.io.loadmat(os.path.join(labeldir, '_'.join(os.path.basename(ppi_file).split('_')[-3:])[:-4]+'.mat'))
            axs[0].contour(Cx,Cy,handpick['evalbox'],[0.5], colors="#ffff00",linewidths=0.8)
        axs[0].axis('equal')
        phelp.add_cbar(axs[0].collections[0],fig,axs[0],unit_text = varunit_table['Zh'],size='3%')

        pdata = np.ma.masked_where(rmask,data['inputNF'][:,:,0])
        axs[1].pcolormesh(Cx,Cy,pdata,cmap=cl.zmap,norm=cl.rxnorm)
        # axs[1].plot(Cx[nfloc],Cy[nfloc],'k.',markersize=0.8)
        axs[1].axis('equal')
        phelp.add_cbar(axs[1].collections[0],fig,axs[1],unit_text = '',size='3%')
        fig.savefig(os.path.join(savedir, ppi_id[:-4]+'.png'))
        # bbox = axs[0].get_window_extent().transformed(fig.dpi_scale_trans.inverted())
        # print("Inches:", bbox)  # in inches


        # # Convert to pixels
        # left = int(bbox.x0 * fig.dpi)
        # right = int(bbox.x1 * fig.dpi)
        # bottom = int(bbox.y0 * fig.dpi)
        # top = int(bbox.y1 * fig.dpi)
        # xl=axs[0].get_xlim()
        # yl=axs[0].get_ylim()
        # print(f"Axes pixel box: left={left}, right={right}, xlim: {xl[0]},{xl[1]}")
        # print(f"Axes pixel box: bottom={bottom}, top={top}, ylim: {yl[0]},{yl[1]}")
        # exit()
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("Usage: python NF_prep_Krita.py [plot|check]")
        sys.exit(1)

    mode = sys.argv[1].lower()
    # nfiles = int(sys.argv[2]) if len(sys.argv) >= 3 else None

    # if nfiles is None:
    #     nfiles = len(fn)
    # else:
    #     nfiles = min(nfiles,len(fn))

    if mode == "plot":
        nffig_proc(config["Settings"]["case_name"])
    elif mode == "check":
        nffig_proc(config["Settings"]["case_name"],True)
    else:
        print(f"Unknown mode: {mode}")
        print("Valid options are: plot, check")
        sys.exit(1)